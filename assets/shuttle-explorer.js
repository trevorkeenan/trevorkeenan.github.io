(function () {
  ...
  Explorer.prototype.buildCurlScript = function (rows) {
    ...
    return [
      ...
      "fi",
      "",
      "echo \"Starting bulk download of $TOTAL files (CONCURRENCY=$CONCURRENCY, MAX_ATTEMPTS=$MAX_ATTEMPTS)\" | tee -a \"$LOGFILE\"",
      "",
      "download_one() {",
      "  local url=\"$1\"",
      "  local outdir=\"$2\"",
      ...
      "    {} \"$OUTDIR\" \"$LOGFILE\" \"$attempt\" \"$SUCCESS_FILE\" \"$failed_out\" || true",
      "",
