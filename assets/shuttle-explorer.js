(function () {
  ...
  function buildSurfacedCoverageSummary(products) {
    return (Array.isArray(products) ? products : []).map(function (product) {
      return productFamilyDisplayName(product && product.productFamily, false) + ": " + String(product && product.coverageLabel || "\u2014");
    }).join(" \u00b7 ");
  }
  ...
  function renderSurfacedCoverageHtml(row) {
    ...
    content = products.map(function (product) {
      return "<span class=\"shuttle-explorer__coverage-item\"><strong>" +
        escapeHtml(productFamilyDisplayName(product && product.productFamily, false)) +
        ":</strong> " +
  ...
  function buildRowDownloadOptions(row, canAmeriFluxDownload) {
    return getSurfacedProductsForRow(row).map(function (product) {
      ...
      var option = {
        product: product,
        displayLabel: productFamilyDisplayName(product && product.productFamily, true),
  ...
  function createLayout(root) {
    ...
    root.innerHTML = [
      ...
      "  <p class=\"shuttle-explorer__muted\">Explore and download a regularly refreshed snapshot of published FLUXNET (ONEFlux-derived) and AmeriFlux BASE (standardized observations) coverage (last updated: <span data-role=\"widget-last-updated-inline\">unavailable</span>). Search by site ID or site name, then open hub-hosted download links directly from the table. This explorer supplements sites available through the FLUXNET Shuttle with additional FLUXNET-family sites hosted elsewhere, and surfaces BASE alongside FLUXNET only when BASE contributes distinct temporal coverage for a site.</p>",
      ...
      "    <li>Data labeled <code>FLUXNET</code> in the year column have been gap-filled and partitioned with the ONEFlux processing pipeline. To locate all ONEFlux-processed datasets in the explorer, choose <code>FLUXNET available</code> in the <code>Availability</code> filter.</li>",
      "    <li>The FLUXNET Shuttle serves data processed with the most recent ONEFlux version and should generally be treated as the highest-quality processed product available here. Choose <code>FLUXNET-Shuttle</code> in the <code>Source</code> filter to view the subset of FLUXNET-format datasets generated with this most up-to-date processing.</li>",
      "    <li>The explorer includes both gap-filled and partitioned data, labeled <code>FLUXNET</code>, and non-gap-filled, non-partitioned observations, labeled <code>BASE</code>.</li>",
      "    <li>For some sites, the <code>BASE</code> product currently extends to years that are not yet available in the corresponding <code>FLUXNET</code> product.</li>",
      "    <li>The bulk-download scripts may require <code>jq</code> if neither <code>jq</code> nor <code>python3</code> is already installed. <code>jq</code> is a lightweight command-line JSON parser used by the script workflow.</li>",
      "  </ul>",
      "</aside>",
