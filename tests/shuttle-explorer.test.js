...
function makeJapanFluxRow(overrides) {
  return makeCatalogRow(Object.assign({
    ...
    first_year: 1999,
    last_year: 2023,
    download_link: 'https://ads.nipr.ac.jp/dataset/A20241022-021',
    download_mode: 'landing_page',
    source_label: 'JapanFlux',
    source_reason: 'Available from JapanFlux2024 (Ueyama et al., 2025, ESSD) via ADS. CC BY 4.0.',
    source_origin: 'japanflux_direct'
  }, overrides || {}));
}

test('JapanFlux rows are ingested into the merged snapshot with correct data_hub and source_network', () => {
  const merged = hooks.mergeCatalogRows(
    [],
    [],
    [],
    ...
    [],
    [
      makeJapanFluxRow({ site_id: 'JP-Tak' }),
      makeJapanFluxRow({ site_id: 'JP-Ozm', site_name: 'Oizumi Urban Park', vegetation_type: 'URB' })
    ]
  );
  const bySite = Object.fromEntries(merged.rows.map((row) => [row.site_id, row]));

  assert.equal(merged.japanFluxOnlySites, 2);
  assert.equal(merged.japanFluxSuppressedByPrecedence, 0);
  assert.equal(bySite['JP-Tak'].data_hub, 'JapanFlux');
  assert.equal(bySite['JP-Tak'].source_network, 'JapanFlux');
  assert.equal(bySite['JP-Tak'].source_origin, 'japanflux_direct');
  assert.equal(bySite['JP-Ozm'].data_hub, 'JapanFlux');
  assert.equal(bySite['JP-Ozm'].vegetation_type, 'URB');
});

test('JapanFlux rows are suppressed by higher-precedence Shuttle rows', () => {
  const shuttleRow = makeCatalogRow({
    site_id: 'JP-Tak',
    site_name: 'Takayama Shuttle',
    country: 'JP',
    data_hub: 'ICOS',
    network: 'FLX',
    source_network: 'FLX',
  ...
  assert.equal(merged.japanFluxSuppressedByPrecedence, 1);
  assert.equal(merged.japanFluxOnlySites, 0);
  const takRows = merged.rows.filter((r) => r.site_id === 'JP-Tak');
  assert.equal(takRows.length, 1);
  assert.equal(takRows[0].source_origin, 'shuttle');
});

test('JapanFlux rows are suppressed by higher-precedence ICOS-direct rows', () => {
  const icosRow = makeCatalogRow({
    site_id: 'JP-Ozm',
    site_name: 'Oizumi ICOS',
    country: 'JP',
    data_hub: 'ICOS',
    network: 'FLX',
    source_network: 'FLX',
  ...
  const merged = hooks.mergeCatalogRows(
    ...
    [],
    [],
    [makeJapanFluxRow({ site_id: 'JP-Ozm' })]
  );

  assert.equal(merged.japanFluxSuppressedByPrecedence, 1);
  assert.equal(merged.japanFluxOnlySites, 0);
  const ozmRows = merged.rows.filter((r) => r.site_id === 'JP-Ozm');
  assert.equal(ozmRows.length, 1);
  assert.equal(ozmRows[0].source_origin, 'icos_direct');
});

test('Existing Shuttle/AmeriFlux/ICOS behavior is unchanged when JapanFlux rows are empty', () => {
  const shuttleRow = makeCatalogRow({
    site_id: 'US-Test',
    data_hub: 'AmeriFlux',
    network: 'AmeriFlux',
  ...
  const mergedWithout = hooks.mergeCatalogRows([shuttleRow], [], [], []);
  const mergedWith = hooks.mergeCatalogRows([shuttleRow], [], [], [], [], []);

  assert.equal(mergedWith.rows.length, mergedWithout.rows.length);
  assert.equal(mergedWith.rows[0].site_id, 'US-Test');
  assert.equal(mergedWith.japanFluxOnlySites, 0);
});

test('JapanFlux source filter tag is assigned correctly', () => {
  const row = makeJapanFluxRow({ site_id: 'JP-Fjy' });
  const tags = hooks.computeSourceFilterTags(row);
  assert.ok(tags.indexOf('JapanFlux') !== -1, 'JapanFlux tag should be present');
});

test('Source filter options include JapanFlux', () => {
  const options = hooks.uniqueSourceFilterValues();
  assert.ok(options.indexOf('JapanFlux') !== -1, 'JapanFlux should be in source filter options');
});

test('isJapanFluxNetworkRow detects JapanFlux rows correctly', () => {
  assert.equal(hooks.isJapanFluxNetworkRow(makeJapanFluxRow()), true);
  assert.equal(hooks.isJapanFluxNetworkRow(makeCatalogRow({ data_hub: 'JapanFlux' })), true);
  assert.equal(hooks.isJapanFluxNetworkRow(makeCatalogRow({ source_origin: 'japanflux_direct' })), true);
  assert.equal(hooks.isJapanFluxNetworkRow(makeCatalogRow()), false);
});

test('JapanFlux rows with direct download participate in Shuttle bulk partition', () => {
  const japanFluxRow = makeJapanFluxRow({
    site_id: 'JP-Fjy',
    download_mode: 'direct',
    download_link: 'https://ads.nipr.ac.jp/download/JapanFlux/A20241022-002/v100/dataset_zip'
  });
  // JapanFlux rows have download_mode "direct" (not "ameriflux_api"),
  // so they should partition into shuttleRows for bulk download.
  const partition = hooks.partitionRowsByBulkSource([japanFluxRow]);
  assert.equal(partition.shuttleRows.length, 1);
  assert.equal(partition.ameriFluxRows.length, 0);
});

test('JapanFlux rows with landing_page download_mode still partition into shuttle bulk', () => {
  const japanFluxRow = makeJapanFluxRow({
    site_id: 'JP-Ozm',
    download_mode: 'landing_page',
...
test('JapanFlux vegetation type normalization works for IGBP codes', () => {
  const row = makeJapanFluxRow({ vegetation_type: 'DNF' });
  const label = hooks.vegetationDisplayLabel(row.vegetation_type);
  assert.ok(typeof label === 'string' && label.length > 0, 'DNF should produce a display label');
});

test('Dedup/precedence does not create duplicate rows when JapanFlux overlaps with Shuttle', () => {
  const shuttleRow = makeCatalogRow({
    site_id: 'JP-Tak',
    data_hub: 'ICOS',
    source_origin: 'shuttle'
  ...
  const merged = hooks.mergeCatalogRows(
    ...
    [],
    [],
    [makeJapanFluxRow({ site_id: 'JP-Tak' }), makeJapanFluxRow({ site_id: 'JP-Api' })]
  );
  const takRows = merged.rows.filter((r) => r.site_id === 'JP-Tak');
  const apiRows = merged.rows.filter((r) => r.site_id === 'JP-Api');
  assert.equal(takRows.length, 1, 'JP-Tak should appear exactly once');
  assert.equal(apiRows.length, 1, 'JP-Api should appear exactly once');
  assert.equal(apiRows[0].data_hub, 'JapanFlux');
});
